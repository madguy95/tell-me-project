<template>
  <div class="container-fluid bg-white position-relative pt-3 pb-3">
    ...
    <b-form-file v-model="imageFile" @change="handleImageUpload" class="mb-3"></b-form-file>
    ...
    <b-list-group class="mb-4">
      <b-list-group-item v-for="(building, buildingIndex) in buildings" :key="buildingIndex">
        <div class="d-flex justify-content-between align-items-center">
          <strong>{{ building.name }}</strong>
          <div>
            <b-button size="sm" variant="warning" @click="editNode('building', buildingIndex)">Sửa</b-button>
            <b-button size="sm" variant="danger" @click="deleteNode('building', buildingIndex)">Xóa</b-button>
          </div>
        </div>
        <b-list-group class="mt-2">
          <b-list-group-item v-for="(floor, floorIndex) in building.floors" :key="floorIndex">
            <div class="d-flex justify-content-between align-items-center">
              <span>{{ floor.name }}</span>
              <div>
                <b-button size="sm" variant="warning" @click="editNode('floor', buildingIndex, floorIndex)"
                  >Sửa</b-button
                >
                <b-button size="sm" variant="danger" @click="deleteNode('floor', buildingIndex, floorIndex)"
                  >Xóa</b-button
                >
              </div>
            </div>
            <b-list-group class="mt-2">
              <b-list-group-item v-for="(room, roomIndex) in floor.rooms" :key="roomIndex">
                <div class="d-flex justify-content-between align-items-center">
                  <span>{{ room.name }}</span>
                  <div>
                    <b-button
                      size="sm"
                      variant="warning"
                      @click="editNode('room', buildingIndex, floorIndex, roomIndex)"
                      >Sửa</b-button
                    >
                    <b-button
                      size="sm"
                      variant="danger"
                      @click="deleteNode('room', buildingIndex, floorIndex, roomIndex)"
                      >Xóa</b-button
                    >
                  </div>
                </div>
            ...
            <b-button size="sm" variant="success" class="mt-2" @click="addNode('room', buildingIndex, floorIndex)"
              >Thêm Phòng</b-button
            >
          </b-list-group-item>
        </b-list-group>
        <b-button size="sm" variant="success" class="mt-2" @click="addNode('floor', buildingIndex)">Thêm Tầng</b-button>
      </b-list-group-item>
    </b-list-group>
    <b-button variant="primary" @click="addNode('building')">Thêm Tòa Nhà</b-button>
  ...
  methods: {
    ...
    editNode(type, buildingIndex, floorIndex = null, roomIndex = null) {
      if (type === 'building') {
        ...
        this.currentEditNode = this.buildings[buildingIndex].floors[floorIndex].rooms[roomIndex]
      }
    ...
    deleteNode(type, buildingIndex, floorIndex = null, roomIndex = null) {
      if (type === 'building') {
        ...
        this.buildings[buildingIndex].floors[floorIndex].rooms.splice(roomIndex, 1)
      }
    }
  },
  created() {
