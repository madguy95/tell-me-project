<template>
  <div class="container-fluid bg-white position-relative pt-3 pb-3">
    ...
    <b-button @click="addMainItem" variant="primary" class="mb-3">Thêm Loại thủ tục</b-button>
    <b-list-group>
      <b-list-group-item v-for="(mainItem, mainIndex) in mainItems" :key="mainItem.id">
        <b-form @submit.prevent="saveMainItem(mainItem, mainIndex)">
          ...
          <b-button type="submit" variant="success" class="mb-2">Lưu Tiêu Đề</b-button>
          <b-button variant="danger" @click="deleteMainItem(mainIndex)">Xóa</b-button>
        </b-form>

        <b-button @click="toggleCollapse(mainIndex)" variant="link">
          {{ mainItem.subItems.length ? `Thủ tục (${mainItem.subItems.length})` : 'Thêm thủ tục ' }}
        </b-button>

        <b-collapse v-model="mainItem.collapse" class="mt-2">
          <b-list-group>
            <b-list-group-item v-for="(subItem, subIndex) in mainItem.subItems" :key="subIndex">
              ...
              <b-form-group label="Hình Ảnh">
                ...
                <img v-if="subItem.imageUrl" :src="subItem.imageUrl" class="img-preview" />
                <b-button
          ...
          <b-button @click="addSubItem(mainIndex)" variant="secondary" class="mt-2">Thêm Mục Con</b-button>
          <b-button @click="saveAllItems(mainItem, mainIndex)" variant="success" class="mt-2"
    ...
    async saveAllItems(mainItem, mainIndex) {
      ...
      for (const subItem of mainItem.subItems) {
        ...
        if (subItem.file) {
          ...
          const storageRefPath = storageRef(storage, `procedure-images/${uuidv4() + '_' + subItem.file.name}`)
