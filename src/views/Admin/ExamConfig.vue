  ...
  <div class="container-fluid bg-white position-relative pt-3 pb-3">
    ...
    <b-tabs v-model="activeTab" fill>
      ...
      <b-tab v-for="(test, tIndex) in tests" :key="tIndex" :title="test.code">
        <b-card class="mt-3">
          <b-form-input v-model="test.name" placeholder="Nhập tên"></b-form-input>
          <b-form-input v-model="test.description" placeholder="Nhập tên"></b-form-input>
          <div v-for="(question, qIndex) in test.questions" :key="qIndex" class="mb-3">
            <b-card-header
              @click="toggleExpand(tIndex, qIndex)"
              class="d-flex justify-content-between align-items-center"
            >
              <span>{{ `${qIndex + 1}. ${question.content}` }}</span>
              <b-button v-if="question.expanded" variant="danger" @click.stop="deleteQuestion(tIndex, qIndex)">
                Xóa câu hỏi
              </b-button>
            </b-card-header>
            ...
            <b-collapse v-model="question.expanded">
              <b-card-body>
                <b-form-group label="Nội dung câu hỏi:">
                  <b-form-input v-model="question.content" placeholder="Nhập nội dung câu hỏi"></b-form-input>
                </b-form-group>

                <h5>Đáp án và trọng số điểm</h5>
                <div class="table-responsive">
                  <b-table :items="question.answers" :fields="answerFields" striped bordered style="width: 100">
                    <template #cell(answer)="data">
                      <b-form-input v-model="data.item.answer" style="min-width: 200px"></b-form-input>
                    </template>
                    <template #cell(score)="data">
                      <b-form-input type="number" v-model="data.item.score"></b-form-input>
                    </template>
                    <template #cell(actions)="data">
                      <b-button variant="danger" @click="deleteAnswer(tIndex, qIndex, data.index)">Xóa</b-button>
                    </template>
                  </b-table>
                </div>
                <b-button variant="success" @click="addAnswer(tIndex, qIndex)">Thêm đáp án</b-button>
              </b-card-body>
            </b-collapse>
          </div>

          <b-button variant="primary" @click="addQuestion(tIndex)">Thêm câu hỏi mới</b-button>
        </b-card>
        <b-card class="mt-3">
          <h5>Quản lý mức độ đánh giá {{ test.name }}</h5>
          <div class="table-responsive">
            <b-table :items="test.evaluations" :fields="evaluationFields" striped bordered responsive>
              <template #cell(level)="data">
                <b-form-input type="number" v-model="data.item.level" disabled></b-form-input>
              </template>
              <template #cell(minScore)="data">
                <b-form-input type="number" v-model="data.item.minScore"></b-form-input>
              </template>
              <template #cell(description)="data">
                <b-form-textarea v-model="data.item.description"></b-form-textarea>
              </template>
              <template #cell(actions)="data">
              ...
              <template #row-details="{ item }">
                <b-card v-if="item._showDetails">
                  ...
                  <quill-editor v-model="item.content" :options="editorOptions" />
                </b-card>
              </template>
            </b-table>
          </div>
          ...
          <b-button
            ...
            >Xóa</b-button
          >
          <b-button variant="success" @click="addEvaluation(test.code)">Thêm mức</b-button>
        </b-card>
      </b-tab>
      ...
      <b-tab title="Đánh giá">
        <b-card class="mt-3">
          ...
          <div class="table-responsive">
            <b-table :items="generalEvaluations" :fields="generalEvaluationFields" striped bordered responsive>
              <template #cell(type)="data">
                <b-form-input type="number" v-model="data.item.level" disabled></b-form-input>
              </template>
              <template #cell(limitLevel)="data">
                <b-form-input type="number" v-model="data.item.limitLevel"></b-form-input>
              </template>
              <template #cell(actions)="data">
                <b-button variant="danger" @click="toggleRowDetails(data)"
                  >Nội dung
    ...
    <!-- Save button -->
    <b-button variant="success" class="mt-4" @click="saveChanges">Lưu thay đổi</b-button>
  </div>
</template>

<script>
import { db } from '@/plugins/firebaseConfig'
import { collection, addDoc, getDocs, updateDoc, doc, query, limit } from 'firebase/firestore'
...
export default {
      ...
      tests: [
        {
          ...
          questions: [
            ...
            {
              content: 'Cảm giác sợ sệt như thể có điều gì đó tồi tệ có thể xảy ra',
              expanded: false,
              answers: [
                { answer: 'Không ngày nào', score: 0 },
                { answer: 'Vài ngày', score: 1 },
        ...
        },
        {
          ...
          questions: [
            ...
            {
              content: 'Khó vào giấc ngủ, khó ngủ thẳng giấc hoặc ngủ quá nhiều.',
              expanded: false,
              answers: [
                { answer: 'Không có', score: 0 },
                { answer: 'Vài ngày', score: 1 },
            ...
            {
              content: 'Khó tập trung vào mọi việc, chẳng hạn như đọc báo hoặc xem truyền hình.',
              expanded: false,
              answers: [
                { answer: 'Không có', score: 0 },
                { answer: 'Vài ngày', score: 1 },
            ...
            {
              content: 'Nghĩ rằng mình nên chết hay muốn tự làm tổn thương bản thân theo cách nào đó',
              expanded: false,
              answers: [
                { answer: 'Không có', score: 0 },
                { answer: 'Vài ngày', score: 1 },
        ...
        },
        {
          ...
          questions: [
            {
              content: 'Khó ngủ, ví dụ khó đi vào giấc ngủ, dễ bị tỉnh giấc hoặc thức dậy sớm?',
              expanded: false,
              answers: [
  ...
  methods: {
    toggleExpand(tIndex, qIndex) {
      this.tests[tIndex].questions[qIndex].expanded = !this.tests[tIndex].questions[qIndex].expanded
    },
